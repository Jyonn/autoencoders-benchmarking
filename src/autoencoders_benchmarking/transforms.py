    ...
    def transform(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            for start in range(0, matrix.shape[0], self.config.batch_size):
                ...
                representation = _select_representation(export, self.output_representation)
...
def _select_representation(export: Any, representation: str) -> torch.Tensor:
    mapping = {
    ...
    value = mapping.get(representation)
    if value is None:
        available = ", ".join(sorted(key for key, item in mapping.items() if item is not None))
        raise ValueError(
            f"Representation {representation!r} is unavailable for this model export. Available: {available or 'none'}."
        )
    if not isinstance(value, torch.Tensor):
        raise TypeError(f"Expected representation {representation!r} to be a torch.Tensor.")
    return value
